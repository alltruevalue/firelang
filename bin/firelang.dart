...
void main() async {
  ...
  if (!configDir.existsSync()) {
    developer.log('❌ Error: "config/" folder not found. Please ensure your project structure is correct.');
    exit(1);
  }

  final Map<String, dynamic> remoteConfigData = {"conditions": [], "parameters": {}};

  // Get all parameter folders inside "config/"
  List<String> params = configDir.listSync().whereType<Directory>().map((dir) => dir.path.split('/').last).toList();

  if (params.isEmpty) {
    developer.log('❌ Error: No parameter directories found inside "config/". Ensure at least one parameter folder exists.');
    exit(1);
  }
  ...
  for (String param in params) {
    ...
    List<File> languageFiles = paramDir.listSync().whereType<File>().where((file) => file.path.endsWith('.json')).toList();

    if (languageFiles.isEmpty) {
      developer.log('❌ Error: No language JSON files found in "config/$param". Each parameter must have at least one valid language.');
      exit(1);
    }
    ...
    if (defaultJson.isEmpty) {
      developer.log('❌ Error: Missing required "default.json" or "en.json" in "config/$param". Each parameter must have at least one default language.');
      exit(1);
    }
    ...
    for (String locale in supportedLocales) {
      ...
      if (missingKeys.isNotEmpty || extraKeys.isNotEmpty) {
        developer.log('❌ Error: Inconsistent keys in "$locale.json" under "config/$param".');
        if (missingKeys.isNotEmpty) developer.log('   Missing keys: $missingKeys');
    ...
    for (String locale in supportedLocales.where((l) => l != defaultLanguage)) {
      if (translations.containsKey(locale)) {
        paramConfig["conditionalValues"][locale] = {"value": jsonEncode(translations[locale])};
      }
    }
    ...
    for (String locale in supportedLocales.where((l) => l != defaultLanguage)) {
      remoteConfigData["conditions"].add({"name": locale, "expression": "device.language in ['${locale.replaceAll('_', '-')}']", "tagColor": "BLUE"});
    }
  }

  // Generate properly formatted Remote Config JSON in the project root
  File remoteConfigFile = File('remoteconfig.template.json');
  remoteConfigFile.writeAsStringSync(JsonEncoder.withIndent('  ').convert(remoteConfigData));
  developer.log('✅ remoteconfig.template.json generated successfully at project root!');
...
Future<void> deployToFirebase() async {
  ...
  var deployResult = await Process.run('firebase', ['deploy', '--only', 'remoteconfig'], runInShell: true);
  if (deployResult.exitCode != 0) {
    developer.log('❌ Error deploying Firebase Remote Config:\n${deployResult.stderr}\nOutput: ${deployResult.stdout}');
    exit(1);
  }
...
void generateLangDart() {
  ...
  File remoteConfigFile = File('${projectRoot.path}/remoteconfig.template.json');

  if (!remoteConfigFile.existsSync()) {
    developer.log('❌ Error: remoteconfig.template.json not found. Cannot generate lang.dart.');
    exit(1);
  }

  // Read Remote Config JSON
  Map<String, dynamic> remoteConfigData = jsonDecode(remoteConfigFile.readAsStringSync());
  ...
  Map<String, dynamic> defaultValues = jsonDecode(remoteConfigData["parameters"][paramKey]["defaultValue"]["value"]);
...
""";
  ...
  developer.log('✅ lib/lang.dart generated successfully in the user’s project!');
}
